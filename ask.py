    ...
    def generateWhere(self, sent):
        ...
        for chunk in sent.noun_chunks:
            if chunk.root.ent_type_ in set(["GPE", "LOC"]) and chunk.root.dep_ == "pobj":
                ...
                # initialize whereVerb to be where root token
                whereVerbList.append(chunk.root)

        if len(whereChunk) == 0:
            return
        # possibly make a where question for each where chunk found
        for chunk_i in range(len(whereChunk)):
